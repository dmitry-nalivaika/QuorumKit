...
1. **Mirror drift across uncovered surfaces** — `templates/src/pipelines/`, `.github/agents/`, and 14 overlapping workflows have no parity gate; one surface has already drifted.
...
3. **Given** a consumer running `installer/init.sh --ai=both`, **When** the install finishes, **Then** zero engine source files (`engine/orchestrator/**`, `engine/dashboard/**`) exist in the consumer repo, but `src/`, `.claude/`, `.github/`, and seed docs are present.
...
1. **Given** a PR that edits `.github/agents/developer-agent.md` without regenerating the Copilot mirror, **When** CI runs, **Then** `verify-mirror.sh` check **M1** fails with the exact missing/diverged file path.
2. **Given** a PR that creates `templates/src/pipelines/foo.yml`, **When** CI runs, **Then** check **M4** fails with message *"`templates/src/pipelines/` MUST NOT exist (ADR-006 §3 — pipelines are not mirrored)"*.
...
5. **Given** a PR that adds `.github/agents/new-agent.md` without populating `.claude/agents/new-agent.md` and `.github/instructions/new-agent.instructions.md`, **When** CI runs, **Then** check **M7** fails (Principle IV self-host parity).
...
1. **Given** the post-migration repo root, **When** a contributor lists top-level folders, **Then** they see `engine/`, `installer/`, `templates/`, `src/`, `.claude/`, `.github/`, `specs/`, `docs/` with no ambiguous names like `scripts/` containing both engine and installer code.
...
3. **Given** every file in `.github/agents/`, **When** check **M7** runs, **Then** it confirms a counterpart exists in both `.claude/agents/` and `.github/instructions/`.
...
- **FR-005**: `templates/src/pipelines/` MUST NOT exist; pipelines are read directly from `src/pipelines/` per ADR-006 §3.
...
- **FR-012**: The Action MUST read pipeline DSL, runtime registry, and identity registry from the consumer's local `src/` directory at runtime; no engine state may be embedded in the Action itself.
...
- **FR-015 (M4)**: CI MUST fail if `templates/src/pipelines/` exists.
...
- **FR-018 (M7)**: CI MUST fail if any `.github/agents/<x>.md` lacks a counterpart in `.claude/agents/` OR `.github/instructions/` (self-host Principle IV parity).
...
- **FR-022**: `installer/init.sh` MUST copy `src/pipelines/` directly from the SoT package; no intermediate template tree is involved.
...
- **FR-027**: `BROWNFIELD_GUIDE.md` and `INIT.md` MUST be updated to reflect new paths (`installer/init.sh`, `engine/`, no `templates/src/pipelines/`).
...
- **SC-004**: Both `.claude/agents/` and `.github/instructions/` in the SoT repo contain a counterpart for every file in `.github/agents/` (Principle IV self-host parity).
...
- **Package payload**: The set of files and directories that `installer/init.sh` copies into a consumer repo. Includes `src/`, `templates/seed/`, `templates/github/`, and select root-level docs. Does NOT include `engine/`, `installer/`, `dashboard/` source.
...
- **Mirror surface**: A directory pair where one is the canonical source of truth and the other is a generated mirror. The four surfaces post-migration: (1) `.github/agents/` ↔ `.github/instructions/`, (2) `.github/agents/` ↔ `.claude/agents/`, (3) `templates/github/workflows/` ↔ `.github/workflows/` (overlapping subset only), (4) `.github/agents/` ↔ enforcement that `.github/agents/` does NOT exist.
...
- **Renaming `src/` itself**: the SoT root stays `src/` per ADR-006.
...
| VII. Simplicity / YAGNI | Three folders deleted (`templates/src/pipelines/`, `.github/agents/`); no new abstractions introduced. |
