...
- **FR-002**: Pipelines MUST be declared as YAML files under `.apm/pipelines/*.yml` and MUST carry an explicit `schema_version` field. Files lacking the field MUST be interpreted as `schema_version: "1"` and processed via the v1 backward-compatibility adapter. The `.apm/` tree is the single source of truth for pipeline files; the orchestrator MUST read pipelines from `.apm/pipelines/` regardless of which agent runtime is selected (per ADR-006). Copilot-tree mirroring of any related artifacts is generated by `scripts/init.sh` and verified by `scripts/verify-mirror.sh` in CI.
