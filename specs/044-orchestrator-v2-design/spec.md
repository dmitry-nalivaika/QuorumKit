...
- Given a PR modifies any file under `src/pipelines/`
  When CI runs
...
- **FR-002**: Pipelines MUST be declared as YAML files under `src/pipelines/*.yml` and MUST carry an explicit `schema_version` field. Files lacking the field MUST be interpreted as `schema_version: "1"` and processed via the v1 backward-compatibility adapter. The `src/` tree is the single source of truth for pipeline files; the orchestrator MUST read pipelines from `src/pipelines/` regardless of which agent runtime is selected (per ADR-006). Copilot-tree mirroring of any related artifacts is generated by `scripts/init.sh` and verified by `scripts/verify-mirror.sh` in CI.
...
- **FR-007**: A **runtime registry** file (canonical path: `src/runtimes.yml`, single source of truth per ADR-006) MUST allow declaring named runtimes. Each runtime declaration MUST include: a unique `name`, a `kind` discriminator drawn from the **kind allowlist** defined in ADR-005, an `endpoint` reference, a `credential_ref` pointing to a secret name (never an inline secret), and optional `model`, `region`, and free-form `parameters` block. Per ADR-005, v2 ships with two enabled kinds (`claude`, `copilot`); the names `azure-openai`, `bedrock`, `ollama`, and `custom` are **reserved** and the validator MUST reject any registry that uses them with error `RUNTIME_KIND_NOT_ENABLED` until a per-kind ADR is merged. Each enabled kind delegates to a runtime adapter at `scripts/orchestrator/runtimes/<kind>.js` exposing a single `invoke(context)` function; the orchestrator MUST contain no kind-specific code paths outside the adapter modules.
...
- **FR-014**: All orchestrator-meaningful labels (`agent:*`, `status:*`, `loop:*`, `gate:*`, etc.), all message outcomes, and all transition triggers MUST be declared in a single canonical regulation document at `docs/AGENT_PROTOCOL.md` (single source of truth, not mirrored — both runtimes link to the same path per ADR-006). The orchestrator MUST refuse to process any pipeline file that references an identifier not declared there.
- **FR-015**: The orchestrator MUST maintain, per active v2 run, a two-channel comment model on the triggering issue/PR (per ADR-004, which supersedes ADR-002): (a) an **append-only audit channel** writing one new `<!-- apm-pipeline-state: {…} -->` comment per state transition — these comments are authoritative, never edited, never deleted, and the run state MUST be reconstructible from them alone; and (b) a **single mutable live-status comment** tagged `<!-- apm-pipeline-status: {"runId":"<uuid>"} -->` edited in place on each transition with a derived human-readable view (current step, iteration, runtime, ETA-to-budget). The live-status comment is a projection; if it is missing or corrupted the orchestrator MUST recreate it from the audit channel without data loss. No other orchestrator-authored comment shapes are permitted on the issue thread. v1 runs continue to use the audit channel only (no live-status comment).
...
- [ ] A user can register an Azure OpenAI runtime in `src/runtimes.yml`, assign it to one agent, and observe in the audit timeline that the agent invocation used that runtime — without editing any GitHub Actions workflow file.
...
- [ ] The regulation document at the canonical path declares every label, message outcome, and transition trigger referenced anywhere in `src/pipelines/` and the orchestrator source; a CI check enforces this.
...
The following items were resolved by Architect ADRs during review of this spec
(see `docs/architecture/`):

- [RESOLVED — ADR-004] State-comment model (mutable live-status vs. immutable
  audit history). Hybrid two-channel model adopted; ADR-002 superseded.
- [RESOLVED — ADR-005] Runtime registry kind enum. Interface-only registry; v2
  ships `claude` + `copilot` only; `azure-openai`, `bedrock`, `ollama`, `custom`
  reserved pending per-kind ADR.
- [RESOLVED — ADR-006] Dual-runtime source-of-truth and synchronisation strategy
  for `src/pipelines/`, `src/runtimes.yml`, and `docs/AGENT_PROTOCOL.md`.
  `src/` is canonical; Copilot tree mirroring is generated by `init.sh` and
  verified in CI.
