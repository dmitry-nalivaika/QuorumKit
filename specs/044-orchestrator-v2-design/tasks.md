...
- [ ] **T0.3** Add empty test scaffolds (one file per new module) so the suite
  compiles and CI fails fast on missing implementations.

## Phase 1 — Foundations (pure modules, no GitHub I/O)

- [ ] **T1.1** `schemas/apm-msg.schema.json` + tests covering required fields,
  outcome enum, per-outcome payload variants. [FR-012]
- [ ] **T1.2** `apm-msg-parser.js` — extract single fenced block, JSON.parse,
...
- [ ] **T1.3** `dedup-key.js` — pure switch over the ADR-007 §1 trigger table;
  fixture-payload tests for each trigger; `workflow_dispatch` returns `null`. [FR-026, ADR-007 §1]
- [ ] **T1.4** `loop-budget.js` — `evaluate(state, transition, now)` returning
...
- [ ] **T1.5** `runtimes/_retry.js` — bounded exponential backoff + jitter,
  configurable max attempts. Tests use a fake clock and asserts attempt count
  on 5xx/429/timeout/success. [ADR-007 §8, FR-030]
- [ ] **T1.6** `redaction.test.js` — guarantees no helper serialises a value
...
- [ ] **T1.7** Skeleton of `docs/AGENT_PROTOCOL.md` listing the document's
...
- [ ] **T2.1** Update `schemas/pipeline.schema.json` — add `schema_version`
...
- [ ] **T2.2** `pipeline-validator.js` — pure semantic validator. Defects:
...
- [ ] **T2.3** `pipeline-loader.js` — read `schema_version`; route to v1
...
- [ ] **T2.4** `pipeline-validator-cli.js` — exits non-zero with one error per
  defect; accepts a file or directory. Snapshot tests. [FR-020]
- [ ] **T2.5** Add `regulation-lint.js` — parses `docs/AGENT_PROTOCOL.md` into
...
- [ ] **T3.1** `schemas/runtimes.schema.json` + Ajv tests. [FR-007]
- [ ] **T3.2** `runtime-registry.js` — load `.apm/runtimes.yml`, validate
...
- [ ] **T3.7** `agent-invoker.js` refactor — looks up runtime via registry,
...
- [ ] **T4.7** `timeline-reconstructor.js` — given an issue's audit comments,
...
- [ ] **T5.1** `index.js` rewire — single `runOrchestrator` handles both v1
...
- [ ] **T5.2** Approval gate path preserved through the v2 dispatcher. Test:
  v1 release-pipeline `approval: required` behaviour byte-identical. [FR-018]
- [ ] **T5.3** Per-step timeout enforcement (default 60 min) → on timeout
...
- [ ] **T5.4** Loop-budget exhaustion path — terminate with
...
- [ ] **T5.5** Dashboard webhook payload — include loop edge identifier and
...
- [ ] **T7.1** Ship `.apm/runtimes.yml`, `.apm/agent-identities.yml`,
