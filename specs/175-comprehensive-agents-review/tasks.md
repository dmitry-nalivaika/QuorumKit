...
- [ ] **T-01** Update `docs/AGENT_PROTOCOL.md` Section 2 with extended `apm-msg` v2 field reference table and FR-002/FR-003 comment format specification *(FR-024)*
- [ ] **T-02** Update `engine/orchestrator/schemas/apm-msg.schema.json`: add optional properties `event_type`, `pipeline_id`, `issue`, `pr`, `branch`, `timestamp`; change `additionalProperties: false` → `additionalProperties: true` *(FR-008)*
- [ ] **T-03** Commit: `docs(protocol): extend apm-msg v2 schema fields and AGENT_PROTOCOL Section 2 (FR-024, FR-008)`

---

## Phase 1 — Branch Guard

- [ ] **T-04** *(RED)* Create `engine/tests/branch-guard.test.sh` with test cases:
...
- [ ] **T-05** *(GREEN)* Implement `scripts/branch-guard.sh`:
...
- [ ] **T-06** Commit: `feat(scripts): add branch-guard.sh with unit tests (FR-010–014)`

---

## Phase 2 — Pipeline Manager

- [ ] **T-07** *(RED)* Create `engine/tests/pipeline.test.sh` with test cases:
...
- [ ] **T-08** *(GREEN)* Implement `scripts/pipeline.sh`:
...
- [ ] **T-09** Commit: `feat(scripts): add pipeline.sh lifecycle manager with unit tests (FR-015–022)`

---

## Phase 3 — Orchestrator Extension (FR-023)

- [ ] **T-10** *(RED)* Create `engine/tests/index-pipeline.test.js` (vitest):
...
- [ ] **T-11** *(GREEN)* Update `engine/orchestrator/index.js`:
...
- [ ] **T-12** Confirm full orchestrator test suite still green: `cd engine && npm test`
- [ ] **T-13** Commit: `feat(orchestrator): accept pipeline_id and worktree_path context fields (FR-023)`
...
- [ ] **T-14** `src/agents/triage-agent.md` — Footprint section + `apm-msg` block in triage comment *(FR-001, FR-009)*
- [ ] **T-15** `src/agents/ba-product-agent.md` — Footprint section + Branch Guard invocation note *(FR-001, FR-012)*
- [ ] **T-16** `src/agents/developer-agent.md` — Footprint section + Branch Guard invocation note *(FR-001, FR-012)*
- [ ] **T-17** `src/agents/architect-agent.md` — Footprint section + Branch Guard invocation note *(FR-001, FR-012)*
- [ ] **T-18** `src/agents/reviewer-agent.md` — Footprint section (full review posted as PR review, summary on linked issue) + Branch Guard invocation note *(FR-001, FR-005, FR-012)*
- [ ] **T-19** `src/agents/qa-test-agent.md` — Footprint section (QA Report posted as PR comment, summary on issue) + Branch Guard invocation note *(FR-001, FR-006, FR-012)*
- [ ] **T-20** `src/agents/security-agent.md` — Footprint section (summary on PR; HIGH/CRITICAL also on issue) *(FR-001, FR-007)*
- [ ] **T-21** `src/agents/devops-agent.md` — Footprint section *(FR-001)*
- [ ] **T-22** `src/agents/compliance-agent.md` — Footprint section *(FR-001)*
- [ ] **T-23** `src/agents/release-agent.md` — Footprint section *(FR-001)*
- [ ] **T-24** `src/agents/docs-agent.md` — Footprint section *(FR-001)*
- [ ] **T-25** `src/agents/tech-debt-agent.md` — Footprint section *(FR-001)*
- [ ] **T-26** `src/agents/incident-agent.md` — Footprint section *(FR-001)*
- [ ] **T-27** `src/agents/digital-twin-agent.md` — Footprint section *(FR-001)*
- [ ] **T-28** `src/agents/ot-integration-agent.md` — Footprint section *(FR-001)*
- [ ] **T-29** Commit: `feat(agents): add Agent Footprint sections to all 15 agent definitions (FR-001–009)`

---

## Phase 5 — Handoff

- [ ] **T-30** Run full test suite: `cd engine && npm test` — confirm all green
- [ ] **T-31** Run markdown link check on all modified `.md` files
- [ ] **T-32** Verify no hardcoded secrets or tokens in any committed file
- [ ] **T-33** Push branch and confirm Draft PR exists (or open one): title `[WIP] #175 — Comprehensive Agent Consistency`
- [ ] **T-34** Verify success criteria checklist in `specs/175-comprehensive-agents-review/spec.md` — mark completed items
