...
#   bash /path/to/quorumkit/src/scripts/init.sh [--ai=claude|copilot|both]
#
# Options:
...
#   QUORUMKIT_PACKAGE_DIR=/path/to/quorumkit bash /path/to/quorumkit/src/scripts/init.sh --ai=both
# =============================================================================
set -euo pipefail
...
GITHUB_TMPL="$QUORUMKIT_PACKAGE_DIR/src/.github"
...
install_claude() {
  ...
  for agent in "${UNIVERSAL_AGENTS[@]}"; do
    cp "$QUORUMKIT_PACKAGE_DIR/src/agents/$agent" ".claude/agents/$agent"
  done
  ...
  for skill_name in "${skills_to_install[@]}"; do
    skill_dir="$QUORUMKIT_PACKAGE_DIR/src/skills/$skill_name"
    if [ -d "$skill_dir" ]; then
  ...
  h1 "2. CLAUDE.md"
  if [ ! -f CLAUDE.md ]; then
    cp "$QUORUMKIT_PACKAGE_DIR/src/seed/CLAUDE.md" CLAUDE.md
...
install_copilot() {
  ...
  for agent in "${UNIVERSAL_AGENTS[@]}"; do
    cp "$QUORUMKIT_PACKAGE_DIR/src/agents/$agent" ".github/agents/$agent"
  done
  ...
  INSTR_SRC="$QUORUMKIT_PACKAGE_DIR/src/.github/instructions"
  ...
  if [ ! -f .github/copilot-instructions.md ]; then
    mkdir -p .github
    cp "$QUORUMKIT_PACKAGE_DIR/src/seed/copilot-instructions.md" .github/copilot-instructions.md
...
install_github_templates() {
  ...
  for doc in CONTRIBUTING.md SECURITY.md; do
    if [ ! -f "$doc" ]; then
      cp "$QUORUMKIT_PACKAGE_DIR/src/seed/$doc" "$doc"
