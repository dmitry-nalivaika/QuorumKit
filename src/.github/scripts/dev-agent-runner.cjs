...
function buildSystemPrompt() {
  ...
  return [
    ...
    `code, ask yourself: "what is the minimum file change that satisfies the`,
    ...
    `  - the acceptance criteria can be verified by reading the changed file.`,
    `When tests ARE needed, **use the existing test infrastructure**:`,
    `  - run the project's existing test runner (check the package roots above for`,
    `    a \`scripts.test\` entry first via \`read_file\`),`,
    `  - add new test files alongside existing ones in the same package,`,
    `  - **NEVER** create a new \`package.json\`, install new test frameworks, or`,
    `    overwrite an existing \`package.json\` to add a \`test\` script. If a`,
    `    sub-package has no test runner, run tests from a sibling package that`,
    `    does, or skip the test (per the rule above for trivial fixes).`,
    ``,
